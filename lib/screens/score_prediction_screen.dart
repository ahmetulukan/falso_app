...
class _ScorePredictionScreenState extends State<ScorePredictionScreen> {
  ...
  Future<void> _loadMatches() async {
    ...
    try {
      ...
      if (mounted) {
        // Load saved predictions for this date
  ...
  void _submitPredictions() async {
    // Save to GameStateService (local + Firestore)
    try {
      final gs = await GameStateService.getInstance();
      final dateStr = '${_selectedDate.year}-${_selectedDate.month.toString().padLeft(2, '0')}-${_selectedDate.day.toString().padLeft(2, '0')}';
      for (final entry in _predictions.entries) {
        await gs.savePrediction(entry.key, dateStr, entry.value['home']!, entry.value['away']!);
      }
    } catch (_) {}
    setState(() => _submitted = true);
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: const Text('Tahminlerin kaydedildi! ⚽'), backgroundColor: AppColors.correct, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
    );
  }
                            ...
                            child: Column(
                              children: [
                                ...
                                Container(
                                  ...
                                  child: Text(
                                    m.status == 'NS' ? '${m.date.hour.toString().padLeft(2, '0')}:${m.date.minute.toString().padLeft(2, '0')}' : m.status,
                                    style: TextStyle(color: m.status == 'NS' ? AppColors.textSecondary : AppColors.correct, fontSize: 11, fontWeight: FontWeight.w600),
                                  ),
                                ),
                                const SizedBox(height: 10),
                                Row(
                                  children: [
                                    Expanded(child: Text(m.homeTeam, textAlign: TextAlign.right, style: const TextStyle(color: AppColors.textPrimary, fontSize: 13, fontWeight: FontWeight.w600))),
                                    Padding(
                                      padding: const EdgeInsets.symmetric(horizontal: 10),
                                      child: (m.status == 'FT' || m.status == 'PEN' || m.status == 'AET')
                                        ? Text('${m.homeScore ?? 0} - ${m.awayScore ?? 0}', style: const TextStyle(color: AppColors.primaryOrange, fontSize: 18, fontWeight: FontWeight.bold))
                                        : Row(
                                            mainAxisSize: MainAxisSize.min,
                                            children: [
                                              _scoreWidget(m.id, true, pred['home']!),
                                              const Padding(padding: EdgeInsets.symmetric(horizontal: 4), child: Text('-', style: TextStyle(color: AppColors.textSecondary, fontSize: 16, fontWeight: FontWeight.bold))),
                                              _scoreWidget(m.id, false, pred['away']!),
                                            ],
                                          ),
                                    ),
                                    Expanded(child: Text(m.awayTeam, textAlign: TextAlign.left, style: const TextStyle(color: AppColors.textPrimary, fontSize: 13, fontWeight: FontWeight.w600))),
                                  ],
                                ),
                              ],
                            ),
